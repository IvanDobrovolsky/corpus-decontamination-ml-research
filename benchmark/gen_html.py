...
JS = '''
var DATA = JSON.parse(atob("B64TOKEN"));
var SK = "pr_v6", state = {};
...
function doC(id, v) { state[id] = state[id] || {}; state[id].h = v; save(); render(); }
function doN(id, v) { state[id] = state[id] || {}; state[id].n = v; save(); }
function sf(n) { fN = n; render(); }
function ss(s) { fS = s; render(); }

function render() {
  var dn=0,pp=0,np=0,ag=0,dg=0;
  for(var i=0;i<DATA.length;i++){var s=state[DATA[i].id];if(s&&s.h){dn++;if(s.h==="P")pp++;else np++;if(s.h===(DATA[i].llm_class==="ASSERTING"?"P":"N"))ag++;else dg++;}}
  document.getElementById("stats").innerHTML="<div>Done: <b>"+dn+"/"+DATA.length+"</b></div><div>Propaganda: <b>"+pp+"</b></div><div>Not: <b>"+np+"</b></div><div>Agree LLM: <b>"+ag+"</b></div><div>Disagree: <b>"+dg+"</b></div>";
  document.getElementById("prog").textContent=dn+"/"+DATA.length;

  var ns={};for(var i=0;i<DATA.length;i++)ns[DATA[i].narrative]=1;var nl=Object.keys(ns).sort();
  var fb='<button class="'+(fN==="all"?"on":"")+'" onclick="sf(\'all\')">All</button>';
  for(var i=0;i<nl.length;i++){fb+='<button class="'+(fN===nl[i]?"on":"")+'" onclick="sf(\''+nl[i]+'\')">'+nl[i]+'</button>';}
  fb+=" | ";var sts=["all","pending","done"];for(var i=0;i<sts.length;i++){fb+='<button class="'+(fS===sts[i]?"on":"")+'" onclick="ss(\''+sts[i]+'\')">'+sts[i]+'</button>';}
  document.getElementById("fil").innerHTML=fb;

  var h="";
  for(var i=0;i<DATA.length;i++){
    var it=DATA[i];if(fN!=="all"&&it.narrative!==fN)continue;
    var s=state[it.id]||{};if(fS==="pending"&&s.h)continue;if(fS==="done"&&!s.h)continue;
    var ll=it.llm_class==="ASSERTING"?"PROPAGANDA":"NOT_PROPAGANDA";
    var lt=it.llm_class==="ASSERTING"?"t-prop":"t-not";
    var hl=!s.h?"PENDING":(s.h==="P"?"PROPAGANDA":"NOT_PROPAGANDA");
    var ht=!s.h?"t-pend":(s.h==="P"?"t-prop":"t-not");
    var nv=(s.n||"").replace(/"/g,"&quot;");
    var lr=(it.llm_raw||"").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    h+='<div class="card'+(s.h?" done":"")+'"><div class="hdr"><div><span class="tag t-narr">'+it.narrative+'</span><span class="docid">#'+it.id+' doc='+it.doc_id+'</span><span class="kwds">'+it.keywords.join(", ")+'</span></div><div>LLM: <span class="tag '+lt+'">'+ll+'</span> You: <span class="tag '+ht+'">'+hl+'</span></div></div><div class="reason">'+lr+'</div><div class="text">'+hlKW(it.context,it.keywords)+'</div><div class="btns"><button class="btn-p'+(s.h==="P"?" sel":"")+'" onclick="doC('+it.id+',\'P\')">PROPAGANDA</button><button class="btn-n'+(s.h==="N"?" sel":"")+'" onclick="doC('+it.id+',\'N\')">NOT PROPAGANDA</button><input class="note" placeholder="Notes" value="'+nv+'" onchange="doN('+it.id+',this.value)"></div></div>';
  }
  document.getElementById("cards").innerHTML=h;
}

function xCSV(){var c="id,doc_id,narrative,llm,human,agree,notes\\n";for(var i=0;i<DATA.length;i++){var it=DATA[i],s=state[it.id]||{};var l=it.llm_class==="ASSERTING"?"PROPAGANDA":"NOT_PROPAGANDA";var hu=s.h==="P"?"PROPAGANDA":(s.h==="N"?"NOT_PROPAGANDA":"");var ag=hu?(hu===l?"Y":"N"):"";c+=it.id+","+it.doc_id+","+it.narrative+","+l+","+hu+","+ag+",\\""+((s.n||"").replace(/"/g,'""'))+"\\"\\n";}dl("results.csv",c,"text/csv");}
function xJSON(){var o=[];for(var i=0;i<DATA.length;i++){var it=DATA[i],s=state[it.id]||{};o.push({id:it.id,doc_id:it.doc_id,narrative:it.narrative,llm:it.llm_class==="ASSERTING"?"PROPAGANDA":"NOT_PROPAGANDA",human:s.h==="P"?"PROPAGANDA":(s.h==="N"?"NOT_PROPAGANDA":null),note:s.n||""});}dl("results.json",JSON.stringify(o,null,2),"application/json");}
function dl(n,c,t){var b=new Blob([c],{type:t}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download=n;a.click();}
render();
'''.replace("B64TOKEN", data_b64)
...
<body>
<h1>Propaganda Classification Review (103 samples, full text ~9K chars each)</h1>
<div id="stats"></div>
<div id="fil"></div>
<div id="cards"></div>
